window.onload = function() {
	...
	input.addEventListener('change', function(e){
		...
		function myFunction(xml) {
			...
			var checkForDarwinCoreFormat = /\b(DarwinRecordSet)/gm;
			
			var termMatches = xml.responseText.match(termNames);
			var fieldMatches = xml.responseText.match(fieldValues);
			var formatMatch = xml.responseText.match(checkForDarwinCoreFormat);
			...
			if(formatMatch === null){
				$(".alert-info").after("<div class='alert alert-danger' role='alert'>The file you uploaded is not of Darwin Core format.</div>");
