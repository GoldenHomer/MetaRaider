...
function update() {
  ...
  for(j=0; j < termName.length; j++) {
	if(typeof termValue !== 'undefined' && termValue.length > 0) {
		template = template.concat("<"+ termName[j].substring(0, 1).toLowerCase() + termName[j].substring(1) +">"+ termValue[j] +"</"+ termName[j].substring(0, 1).toLowerCase() + termName[j].substring(1) +">");
	}
  }
